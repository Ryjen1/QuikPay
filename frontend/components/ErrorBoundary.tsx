...
// ─── QuikPayError code map ────────────────────────────────────────────────────
// Mirrors contracts/common/src/error.rs  (repr u32, offset 1001–1040 + 1999)
const QUIKPAY_ERROR_MESSAGES: Record<number, string> = {
...
const FIX_IT_GUIDES: Record<string, FixItGuide> = {
  ...
  "1007": {
    ...
    steps: [
      "The QuikPay protocol has been paused by governance.",
...
/**
 * Extracts a QuikPayError code from an error message/name.
 */
function extractContractErrorCode(error: Error): number | null {
  ...
  for (const re of patterns) {
    ...
    if (m) {
      ...
      if (code in QUIKPAY_ERROR_MESSAGES) return code;
    }
  }
  return null;
}
...
function ErrorFallback({
  ...
  const contractMessage = contractCode
    ? QUIKPAY_ERROR_MESSAGES[contractCode]
    : null;
 ...
 * decodes Soroban QuikPayError contract codes into human-readable messages,
