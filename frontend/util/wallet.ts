...
const SUPPORTED_WALLET_IDS = new Set(["freighter"]);

export const clearUnsupportedWalletSession = () => {
  const walletId = storage.getItem("walletId");
  if (walletId && !SUPPORTED_WALLET_IDS.has(walletId)) {
...
export const connectWallet = async () => {
  await kit.openModal({
    ...
    onWalletSelected: (option: ISupportedWallet) => {
      ...
      if (!SUPPORTED_WALLET_IDS.has(selectedId)) {
