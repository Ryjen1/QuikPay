...
export const verifyQuikPaySignature = (
  req: SecureRequest,
  res: Response,
  next: NextFunction,
) => {
  const signingSecret = secretsBootstrap.getSecret(
    "QUIKPAY_WEBHOOK_SIGNING_SECRET",
  );

  if (!signingSecret) {
    console.error("[Security] ❌ QUIKPAY_WEBHOOK_SIGNING_SECRET not configured");
    return res.status(500).json({ error: "Security configuration missing" });
  }

  const signatureHex = req.get("X-QuikPay-Signature");
  ...
  try {
    ...
    console.warn("[Security] ⚠️ Invalid QuikPay signature");
    return res.status(401).json({ error: "Invalid request signature" });
  } catch (error) {
    console.error("[Security] Error verifying QuikPay signature:", error);
    return res.status(401).json({ error: "Signature verification failed" });
  }
};
