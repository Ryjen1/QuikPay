...
const buildOutgoingPayload = (
  ...
  if (sub.url.includes("discord.com/api/webhooks")) {
    outgoingPayload = {
      embeds: [
        {
          title: `QuikPay Notification: ${eventType.toUpperCase()}`,
      ...
      blocks: [
        {
          ...
          text: {
            type: "plain_text",
            text: `QuikPay Notification: ${eventType.toUpperCase()}`,
          },
        },
        {
          type: "section",
...
const computeQuikPayWebhookSignatureHex = (
  rawBody: Buffer,
...
const attemptDeliveryOnce = async (params: {
  ...
  try {
    const signingSecret = process.env.QUIKPAY_WEBHOOK_SIGNING_SECRET;

    const requestBodyString = JSON.stringify(params.outgoingPayload);
    const signatureHex = signingSecret
      ? computeQuikPayWebhookSignatureHex(
    ...
    const response: any = await webhookBreaker.fire(
      ...
      params.outgoingPayload,
      {
        ...
        headers: signatureHex
          ? {
              "X-QuikPay-Signature": signatureHex,
            }
          : undefined,
      },
    );
