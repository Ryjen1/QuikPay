...
const getQUIKPAY_CONTRACT_ID = () => process.env.QUIKPAY_CONTRACT_ID || "";
...
/**
 * Starts polling the Soroban RPC for QuikPay contract events.
 */
export const startStellarListener = async () => {
  const QUIKPAY_CONTRACT_ID = getQUIKPAY_CONTRACT_ID();

  if (!QUIKPAY_CONTRACT_ID) {
    const errorMsg =
      "[Stellar Listener] ❌ FATAL: QUIKPAY_CONTRACT_ID environment variable is not set. " +
      "The Stellar listener cannot start without a valid contract ID. " +
      "Please set QUIKPAY_CONTRACT_ID in your .env file and restart the server.";
    console.error(errorMsg);
    // In production, this should cause the server to exit or prevent initialization
  ...
  console.log(
    `[Stellar Listener] 📡 Listening for events on contract: ${QUIKPAY_CONTRACT_ID}`,
  );
      ...
      try {
        ...
        const eventsResponse: any = await getGetEventsBreaker().fire({
          ...
          filters: [
            {
              type: "contract",
              contractIds: [QUIKPAY_CONTRACT_ID],
            },
          ],
          limit: 100,
        });
...
const simulateEvents = () => {
  simulationIntervalId = setInterval(() => {
    ...
    const payload = {
      ...
      contractId: "C_SIMULATED_QUIKPAY_CONTRACT",
      type: "contract",
